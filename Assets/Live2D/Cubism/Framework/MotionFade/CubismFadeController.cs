...
namespace Live2D.Cubism.Framework.MotionFade
{
    ...
    public class CubismFadeController : MonoBehaviour, ICubismUpdatable
    {
        ...
        /// <summary>
        /// Layer weights for additive blending.
        /// </summary>
        private float[] _layerWeights;

        /// <summary>
        /// Additive layer flags.
        /// </summary>
        private bool[] _additiveLayers;
        ...
        public void Refresh()
        {
            ...
            _layerWeights = new float[_fadeStates.Length];
            _additiveLayers = new bool[_fadeStates.Length];
            for (int i = 0; i < _layerWeights.Length; i++)
            {
                _layerWeights[i] = 1.0f;
            }

            // Get additive flags from motion controller
            if (_motionController != null)
            {
                for (int i = 0; i < _additiveLayers.Length; i++)
                {
                    _additiveLayers[i] = _motionController.IsLayerAdditive(i);
                }
            }
        }
        ...
        public void OnLateUpdate()
        {
            ...
            for (var i = 0; i < _fadeStates.Length; ++i)
            {
                ...
                bool isAdditive = _additiveLayers != null && i < _additiveLayers.Length && _additiveLayers[i];
                UpdateFade(_fadeStates[i], isAdditive);
            }
        }
        ...
        /// <param name="isAdditive">If true, blend additively instead of override.</param>
        private void UpdateFade(ICubismFadeState fadeState, bool isAdditive = false)
        {
            ...
            for (var i = 0; i < playingMotions.Count; i++)
            {
                ...
                for (var j = 0; j < DestinationParameters.Length; ++j)
                {
                    ...
                    value = Evaluate(
                        ...
                        motionWeight, DestinationParameters[j].Value, isAdditive);
                ...
                for (var j = 0; j < DestinationParts.Length; ++j)
                {
                    ...
                    DestinationParts[j].Opacity = Evaluate(
                            ...
                            fadeInWeight, fadeOutWeight,
                            fadeMotion.ParameterFadeInTimes[index], fadeMotion.ParameterFadeOutTimes[index],
                            motionWeight, DestinationParts[j].Opacity, isAdditive);
                }
        ...
        public float Evaluate(
            ...
            float motionWeight, float currentValue, bool isAdditive = false)
        {
            if (curve.length <= 0)
            {
                return currentValue;
            ...
            return Evaluate(
                ...
                motionWeight, currentValue, isAdditive);
        }
            ...
            float motionWeight, float currentValue, bool isAdditive = false)
        {
            ...
            else
            {
                ...
                finalWeight = fadeInWeight * fadeOutWeight * motionWeight;
            }

            if (isAdditive)
            {
                return currentValue + value * finalWeight;
            }
            return currentValue + (value - currentValue) * finalWeight;
        }
