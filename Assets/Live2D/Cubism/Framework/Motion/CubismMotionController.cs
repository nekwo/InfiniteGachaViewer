...
namespace Live2D.Cubism.Framework.Motion
{
    ...
    public class CubismMotionController : MonoBehaviour
    {
        ...
        /// <summary>
        /// Recreate layers with a new count. Call this before any animations play.
        /// </summary>
        public void RecreateLayers(int count)
        {
            if (count < 1) count = 1;
            if (_motionLayers != null && _motionLayers.Length == count) return;
            ...
            // Create layer mixer
            _layerMixer = AnimationLayerMixerPlayable.Create(_playableGrap, count);

            LayerCount = count;
            _motionLayers = new CubismMotionLayer[LayerCount];
            _motionPriorities = new int[LayerCount];
            _additiveLayers = new bool[LayerCount];
        ...
        public void SetLayerAdditive(int layerIndex, bool isAdditive)
        {
            ...
            _additiveLayers[layerIndex] = isAdditive;
            _layerMixer.SetLayerAdditive((uint)layerIndex, isAdditive);
        }

        /// <summary>
        /// Get whether a layer is additive.
        /// </summary>
        public bool IsLayerAdditive(int layerIndex)
        {
            if (layerIndex < 0 || layerIndex >= LayerCount || _additiveLayers == null)
                return false;
            return _additiveLayers[layerIndex];
        }
